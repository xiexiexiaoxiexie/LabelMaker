...
def run_mask3d(
  ...
  # load point cloud
  input_mesh_path = str(scene_dir / 'single_class'/mesh_name)
  ...
  output_dir = scene_dir / output_folder/mesh_name[0:-4]
...
def run_rendering(
  ...
  # read mesh
  mesh_path = scene_dir / mesh_name
  assert mesh_path.exists()
  mesh = o3d.io.read_triangle_mesh(str(mesh_path))
  mesh_verties = np.asarray(mesh.vertices)
  mesh_vertex_color = np.asarray(mesh.vertex_colors)
  ...
  for i, inst in enumerate(instances):
    ...
    instance_mesh.vertices = o3d.utility.Vector3dVector(mesh_verties[mask])
    instance_mesh.vertex_colors = o3d.utility.Vector3dVector(mesh_vertex_color[mask])
    o3d.io.write_triangle_mesh(str(str(mesh_path)[0:-4]+f'_{i}.ply'), instance_mesh)
